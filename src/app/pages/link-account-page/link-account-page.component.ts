...
export class LinkAccountPageComponent implements OnInit {
  public registerForm!: FormGroup;
  public loading = false;
  public errorMessage: string | null = null;
  public showRegistrationForm = false;
  public matchingDisplayName: string | null = null;
  ...
  private loadParameters(): void {
    ...
    this.matchingDisplayName = this.route.snapshot.queryParamMap.get('matching_display_name');

    if (!this.operationId) {
      this.errorMessage = 'Invalid request. Missing operation ID.';
    }
  }
  ...
  onAcceptLink(): void {
    ...
    this.registerService.acceptLinkAccount(this.operationId)
      .subscribe({
        next: (response) => this.handleSuccess(response.token),
  ...
  private performRegistration(displayName: string, username: string): void {
    ...
    this.registerService.rejectLinkAccount(this.operationId!, displayName, username)
      .subscribe({
        next: (response) => this.handleSuccess(response.token),
        error: (error) => this.handleError(error)
      });
